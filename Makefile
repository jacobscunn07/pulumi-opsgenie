PROJECT_NAME := opsgenie Package
include build/common.mk

PACK             := opsgenie
PACKDIR          := sdk
PROJECT          := github.com/jacobscunn07/pulumi-${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-opsgenie,${REPOSITORY},g' provider/go.mod; \
...
tfgen::
	cd provider && go install -ldflags "-X github.com/jacobscunn07/pulumi-${PACK}/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${TFGEN}

provider:: generate_schema
	cd provider && go generate cmd/${PROVIDER}/main.go
	cd provider && go install -ldflags "-X github.com/jacobscunn07/pulumi-${PACK}/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${PROVIDER}
