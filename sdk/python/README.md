...
First, clone this repo with the name of the desired provider in place of `opsgenie`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-opsgenie
```

Second, replace references to `opsgenie` with the name of your provider:
...
- `opsgenie:apiKey` (environment: `XYZ_API_KEY`) - the API key for `opsgenie`
- `opsgenie:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
